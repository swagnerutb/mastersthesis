...
n_assets = 1
test = multi_Heston_algopy(spot0 = [1 for _ in range(n_assets)],
                           vol0 = [0.1 for _ in range(n_assets)],
                           T_eval = [1,2], #the times at which to evaluate spot
                           T_maturity = [2 for _ in range(n_assets)], #maturities
                           theta = [0.1 for _ in range(n_assets)], #long-run variance
                           kappa = [2.0 for _ in range(n_assets)], #mean reversion rate
                           xi = [0.1 for _ in range(n_assets)], #volatility of volatility
                           rho = [-0.2 for _ in range(n_assets)], # correlation of BMs - asset and vol
                           cov_matrix = np.eye(n_assets), #correlation between different assets (10 combinations)
                           mu = [0 for _ in range(n_assets)], # drift of asset
                           n_t = 100, # total nbr of simulation time-points
                           volMult = [1,1],
                           call_idx = [], #max(call_idx) <= n_assets
                           call_K = [],
                           put_idx = [0], #max(put_idx) <= n_assets
                           put_K = [1])